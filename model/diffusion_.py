...
class DiffusionModel(nn.Module):
    ...
    def forward(self, img):
        ...
        '''
        0920 여기다가 for문써가지고, [:,idx,:,:,:], [:,idx+1, :, :, :]해서 for문 코딩 다시해야함
        '''
        losses = 0
        ...
        for i in range(f-1):
            input_x = x[:, i, :, :, :].squeeze()
            model_out = self.model(input_x, t)
            noise_2_end = noise[:, i+1, :, :, :].squeeze()

            loss = self.loss_fn(model_out, noise_2_end, reduction = 'none')
    ...
    @torch.no_grad()
    def sample_input(self, data, noise_clamp=False, denoised_clamp=False):
        """
        Implementation of `Algorithm2. Sampling`
        """
        x = data
        for _, t_ in tqdm(enumerate(range(self.num_timesteps))):
            t = self.num_timesteps - t_ - 1
            x = self.p_sample(x, x.new_full((16,), t, dtype=torch.long),
                              noise_clamp=noise_clamp,
                              denoised_clamp=denoised_clamp)
        x = unscale_img_linear(x.clamp(min=-1, max=1))
        return x
    ...
    def q_sample(self, x: torch.Tensor, t: torch.Tensor, noise=None):
        ...
        mean = self.alphas_cumprod.gather(-1, t).reshape(-1, 1, 1, 1, 1)
        mean = mean.sqrt() * x
        var = 1 - self.alphas_cumprod.gather(-1, t).reshape(-1, 1, 1, 1, 1)
        return mean + (var ** 0.5) * noise
